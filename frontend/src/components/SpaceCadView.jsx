...
const SpaceCadView = ({ resources = [], onBookResource, isAdmin }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-primary animate-in fade-in duration-500 mt-6">
      ...
      <div className="lg:col-span-2 flex flex-col gap-5">
        ...
        {/* CAD Render Area */}
        <div className="relative aspect-[16/10] bg-raised border border-subtle rounded-[32px] overflow-hidden shadow-xl group flex items-center justify-center glossy-mesh">
          
          {/* Blueprint Grid Lines */}
          <div 
            className="absolute inset-0 opacity-[0.03]" 
            style={{
              backgroundImage: 'radial-gradient(var(--text-primary) 1px, transparent 1px), linear-gradient(to right, var(--text-primary) 1px, transparent 1px), linear-gradient(to bottom, var(--text-primary) 1px, transparent 1px)',
          ...
          <div className="absolute inset-6 border border-subtle rounded-2xl overflow-hidden flex items-center justify-center bg-surface/40 backdrop-blur-md">
            {activeFloor === '1' ? (
              <div className="relative w-full h-full p-4">
                ...
                <div className="absolute inset-4 border border-accent/20 rounded-2xl bg-accent/[0.02] pointer-events-none" />
                  ...
                  return (
                    <div
                      ...
                      className={`absolute rounded-2xl border transition-all duration-500 cursor-pointer flex flex-col justify-between p-4 select-none ${
                        isSelected 
                          ? 'bg-accent/10 border-accent shadow-[0_0_20px_rgba(37,99,235,0.25)] z-20 scale-[1.01]' 
                          : isOutOfOrder 
                          ? 'bg-red-500/10 border-red-500/30 hover:border-red-500/60 shadow-md' 
                          : isMaintenance 
                          ? 'bg-amber-500/10 border-amber-500/30 hover:border-amber-500/60 shadow-md'
                          : 'bg-surface border-subtle hover:bg-raised hover:border-accent/40 hover:shadow-lg'
                      }`}
                      style={{
                        left: space.x,
                        top: space.y,
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] font-black tracking-widest uppercase ${
                            isOutOfOrder ? 'text-red-500' : isMaintenance ? 'text-amber-500' : 'text-accent'
                          }`}>
                            {space.cadId}
                          </span>
                          <span className="text-[9px] font-bold text-muted">{space.baseTemp.toFixed(0)}°C</span>
                        </div>
                        <span className="text-sm font-black text-primary tracking-tight mt-1 truncate">{space.name}</span>
                      </div>
          ...
          <div className="absolute bottom-6 left-6 bg-surface/80 backdrop-blur-md border border-subtle px-4 py-2 rounded-2xl text-[10px] text-secondary font-semibold shadow-md flex items-center gap-2">
      ...
      <div className="flex flex-col gap-5">
        {selectedSpace ? (
          <div className="glass-card rounded-[32px] p-6 shadow-xl hover:border-accent/40 transition-all duration-300 flex flex-col gap-6 relative group overflow-hidden animate-in slide-in-from-right-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Temp Sensor */}
              <div className="bg-surface/50 border border-subtle backdrop-blur-sm p-4 rounded-2xl flex flex-col justify-between shadow-sm">
            ...
            {/* Occupancy metrics */}
            <div className="bg-surface/50 border border-subtle p-4 rounded-2xl flex items-center justify-between shadow-sm">
          ...
          <div className="glass-card border-dashed border-subtle rounded-[32px] p-12 text-center flex flex-col items-center justify-center gap-4 min-h-[300px]">
